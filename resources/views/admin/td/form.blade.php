...
<section class="admin-section">
    <form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="admin-form-stack">
        @csrf
        ...
        <div class="admin-card-block">
            ...
            <div class="admin-form-grid">
                <div class="admin-form-group admin-form-group--full">
                    ...
                    <input type="file" name="document" id="admin-td-source-file" accept=".pdf,.doc,.docx,.txt,.odt,.rtf,.html,.htm">
                    <small>Formats autorisés : PDF, DOC, DOCX, TXT, ODT, RTF, HTML.</small>
                </div>
                @if($isEdit && $td->document_path)
                    <div class="admin-doc-box admin-form-group--full">
                        <strong>Document actuel :</strong>
                        <span>{{ $td->document_name ?: 'Document joint' }}</span>
                        <label class="admin-inline-check"><input type="checkbox" name="remove_document" value="1"> Supprimer le document actuel</label>
                    </div>
                @endif
            </div>
            <div class="admin-actions admin-actions--wrap" style="margin-top:14px;">
                <button type="button" class="btn btn--ghost" id="admin-convert-new-file">Importer et convertir le nouveau fichier</button>
                @if($isEdit && $td->document_path)
                    <button type="button" class="btn btn--ghost" id="admin-convert-current-file">Convertir le document actuel dans l’éditeur</button>
                @endif
                <span class="admin-muted" id="admin-conversion-status">Choisis un nouveau fichier ou convertis le document actuel pour remplir l’éditeur.</span>
            </div>
        </div>

        <div class="admin-card-block" id="editor-zone">
            <h2>Version éditable</h2>
            <p class="admin-muted" style="margin:0 0 12px;">Ici tu peux corriger, reformuler et gérer le TD directement depuis la plateforme.</p>
            <textarea id="admin-td-editor" name="editable_html" rows="12">{{ old('editable_html', $td->editable_html) }}</textarea>
            <input type="hidden" id="admin-td-editor-text" name="editable_text" value="{{ old('editable_text', $td->editable_text) }}">
        ...
        <div class="admin-card-block">
            ...
            <div class="admin-form-grid">
                <div class="admin-form-group admin-form-group--full">
                    ...
                    <textarea id="admin-correction-editor" name="correction_html" rows="8">{{ old('correction_html', $td->correction_html) }}</textarea>
                </div>
                <div class="admin-form-group admin-form-group--full">
                ...
                @if($isEdit && $td->correction_document_path)
                    <div class="admin-doc-box admin-form-group--full">
                        ...
                        <span>{{ $td->correction_document_name ?: 'Corrigé joint' }}</span>
                        <label class="admin-inline-check"><input type="checkbox" name="remove_correction_document" value="1"> Supprimer le document corrigé actuel</label>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="admin-actions admin-actions--end">
            <a href="{{ route('admin.td.index') }}" class="btn btn--ghost">Annuler</a>
            <button type="submit" class="btn btn--primary">{{ $isEdit ? 'Enregistrer les modifications' : 'Créer le TD' }}</button>
        </div>
    </form>
</section>
    ...
    async function handleFile(file, sourceLabel) {
        ...
        try {
            ...
            if (name.endsWith('.pdf')) {
                const buffer = await file.arrayBuffer();
                const text = await convertPdfBuffer(buffer);
                if (!text || !text.trim()) {
                    status.textContent = 'Le PDF ne contient pas de texte exploitable automatiquement. Garde le document source et complète l’éditeur manuellement.';
                    return;
                }
                setEditorContent(convertTextToHtml(text), text);
