    ...
    $displayMessages = $messages->isNotEmpty()
        ...
        : collect([
            (object) [
                ...
                'role_tag' => 'Élève',
                'message' => 'Les quiz m’aident à réviser plus vite avant les contrôles.',
            ],
            (object) [
                'is_anonymous' => true,
                'author_label' => 'Anonyme',
                'role_tag' => 'Parent',
                'message' => 'Je vois mieux la progression de mon enfant semaine après semaine.',
            ],
            (object) [
                'is_anonymous' => false,
                'author_label' => 'Prof Coach',
                'role_tag' => 'Enseignant',
                'message' => 'La plateforme facilite le suivi des exercices et des points faibles.',
            ],
        ]);
@endphp
...
@media (max-width:1024px){.home-hero__grid,.support-wrap{grid-template-columns:1fr}.trust-grid{grid-template-columns:repeat(3,1fr)}}
@media (max-width:720px){.trust-grid{grid-template-columns:repeat(2,1fr)}.home-hero{padding-top:48px}}
</style>
        ...
        <div class="container">
            <div class="home-hero__grid">
                <div class="reveal">
                    ...
                    <h1 class="home-hero__title">{{ $hero['title'] ?? 'Réussissez vos examens avec TIMAH ACADEMY' }}</h1>
                    <p class="home-hero__subtitle">{{ $hero['subtitle'] ?? 'Une plateforme claire, moderne et structurée pour apprendre avec des cours, quiz et TD adaptés à votre classe.' }}</p>
                    ...
                    @if(!empty($hero['reassurance']) && is_array($hero['reassurance']))
                ...
                <div class="classes-grid" data-classes-grid>
                    @forelse($featuredClasses as $index => $class)
                        <article class="class-card" data-class-level="{{ $class->level }}">
                            <div class="class-badges">
                                ...
                                @endif
                                @if($index % 5 === 0)
                                    <span class="class-badge class-badge--new">Nouveau</span>
                                @endif
                            </div>
                            <h3>{{ $class->name }}</h3>
                    ...
                    @empty
                        <article class="class-card">
                            <h3>Contenus bientôt disponibles</h3>
                            <p class="muted">Les classes seront affichées ici dès activation côté administration.</p>
                        </article>
                    @endforelse
                </div>
            </div>
                ...
                <div class="faq-grid">
                    ...
                    @empty
                        <article class="faq-item">
                            ...
                            <p class="muted">Ajoutez des questions/réponses depuis l’espace admin homepage.</p>
                        </article>
                    @endforelse
                </div>
            </div>
