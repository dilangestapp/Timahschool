...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .watermark { position:absolute; inset:36% 0 auto 0; text-align:center; font-size:72px; font-weight:900; color:rgba(26,35,126,.035); transform:rotate(-18deg); letter-spacing:.08em; pointer-events:none; }
        .top { background:linear-gradient(135deg,var(--blue),#173b74 58%,#0f766e); color:#fff; padding:24px 28px; display:flex; justify-content:space-between; gap:20px; align-items:center; }
        ...
        .doc-head { padding:26px 28px 18px; border-bottom:1px solid var(--line); }
        ...
        .foot { border-top:1px solid var(--line); padding:14px 28px; color:var(--muted); font-weight:700; font-size:13px; display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap; }
        @media print { body{background:#fff}.page{box-shadow:none;width:100%;margin:0;border:0;border-radius:0}.actions{display:none}.top{print-color-adjust:exact;-webkit-print-color-adjust:exact}.watermark{display:block} }
...
<div class="page">
    ...
    <header class="top">
        <div class="brand"><div class="logo">{{ $brand['brand_initials'] }}</div><div><h1>{{ $brand['brand_name'] }}</h1><p>{{ $brand['slogan'] }} · {{ $brand['signature'] }}</p></div></div>
    ...
    <section class="doc-head"><h2>{{ $brand['title'] }}</h2><div class="meta">{{ $brand['subtitle'] }} @if($brand['author']) · Enseignant : {{ $brand['author'] }} @endif</div></section>
    <main class="content">
        @if($brand['content_html'])<div class="rich">{!! $brand['content_html'] !!}</div>@endif
        @if($brand['document_name'])<section class="document-box"><strong>Document joint : {{ $brand['document_name'] }}</strong><p>Document publié depuis TIMAH ACADEMY avec identité officielle et QR code.</p><div class="actions"><a href="{{ $embedUrl }}" target="_blank" class="btn btn-primary">Ouvrir le fichier joint</a><button onclick="window.print()" class="btn btn-ghost">Imprimer / PDF</button></div>@if($embedUrl)<iframe src="{{ $embedUrl }}"></iframe>@endif</section>@else<div class="actions"><button onclick="window.print()" class="btn btn-primary">Imprimer / PDF</button></div>@endif
    </main>
