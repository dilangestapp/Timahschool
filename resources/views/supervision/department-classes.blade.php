...
@section('content')
@php
    $linkedClassIds = collect($linkedClassIds ?? [])->map(fn($id) => (int) $id)->all();
...
<div class="dm-wrap">
    <section class="dm-hero">
        ...
        <p>Les classes et matières créées ici restent toutes liées au département. Une nouvelle création n’écrase plus les anciennes.</p>
    ...
    <section class="dm-grid" id="classes">
        <div class="dm-card">
            ...
            <div class="dm-note">Chaque nouvelle classe sera ajoutée au département sans retirer les anciennes.</div>
        ...
        <div class="dm-card">
            <h3>Classes liées au département</h3>
            @forelse($classes as $class)
                <div class="dm-row">
                    <strong>{{ $class->name }}</strong>
                    <small>{{ $class->level ?? 'Niveau non défini' }}</small>
                    @if(in_array((int) $class->id, $linkedClassIds, true))<span class="dm-badge">Liée au département</span>@endif
            ...
            @empty
                <p>Aucune classe liée au département pour le moment.</p>
            @endforelse
        </div>
    </section>
    ...
    <section class="dm-grid" id="matieres">
        <div class="dm-card">
            ...
            <form class="dm-form" method="POST" action="{{ route('department.subjects.store') }}">
                ...
                <label>Description<textarea name="description"></textarea></label>
                <label>Icône<input name="icon" placeholder="📘"></label>
