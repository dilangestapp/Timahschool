...
@section('page_subtitle', 'Gestion des classes et matières du département.')

@section('content')
@php
    $linkedSubjectIds = collect($linkedSubjectIds ?? [])->map(fn($id) => (int) $id)->all();
@endphp
<style>
.dm-wrap{display:grid;gap:18px}.dm-hero{background:#0f2a69;color:#fff;border-radius:24px;padding:20px}.dm-hero h2{margin:6px 0;font-size:2rem}.dm-hero p{color:#dbeafe}.dm-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}.dm-btn{display:inline-flex;align-items:center;justify-content:center;min-height:40px;padding:0 13px;border:0;border-radius:12px;background:#0f2a69;color:#fff;text-decoration:none;font-weight:900;cursor:pointer}.dm-btn--white{background:#fff;color:#0f172a}.dm-btn--green{background:#16a34a}.dm-grid{display:grid;grid-template-columns:360px 1fr;gap:18px}.dm-card{background:#fff;border:1px solid #e2e8f0;border-radius:18px;padding:16px;box-shadow:0 12px 28px rgba(15,23,42,.05)}.dm-form{display:grid;gap:10px}.dm-form label{display:grid;gap:5px;font-weight:800;color:#334155}.dm-form input,.dm-form select,.dm-form textarea{border:1px solid #cbd5e1;border-radius:12px;padding:10px}.dm-row{border:1px solid #e5e7eb;border-radius:14px;padding:12px;margin-bottom:10px;background:#f8fafc}.dm-row strong{display:block;color:#0f172a}.dm-row small{color:#64748b}.dm-inline{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.dm-badge{display:inline-flex;padding:4px 8px;border-radius:999px;background:#dcfce7;color:#166534;font-weight:900;font-size:12px;margin-top:5px}.dm-note{background:#eff6ff;border:1px solid #bfdbfe;color:#1d4ed8;border-radius:14px;padding:10px;font-weight:800}@media(max-width:980px){.dm-grid{grid-template-columns:1fr}.dm-inline{grid-template-columns:1fr}.dm-btn{width:100%}}
</style>
...
<section class="dm-hero">
    ...
    <p>Les classes restent affichées comme avant. La correction multiple concerne les matières afin qu’une nouvelle matière ne remplace plus l’ancienne.</p>
    <div class="dm-actions">
        <a class="dm-btn dm-btn--white" href="{{ route('responsible.department.dashboard') }}">Retour TB département</a>
        <a class="dm-btn dm-btn--green" href="#classes">Classes</a>
        <a class="dm-btn dm-btn--green" href="#matieres">Matières</a>
    </div>
</section>
...
<section class="dm-grid" id="matieres">
    <div class="dm-card">
        ...
        <div class="dm-note">Chaque nouvelle matière sera ajoutée au département sans retirer les anciennes.</div>
        <form class="dm-form" method="POST" action="{{ route('department.subjects.store') }}">
            @csrf
            <label>Nom de la matière<input name="name" required placeholder="Exemple : Électrotechnique"></label>
            <label>Description<textarea name="description"></textarea></label>
            <label>Icône<input name="icon" placeholder="Icône"></label>
            <label>Couleur<input name="color" value="#2563eb"></label>
            <label>Ordre<input type="number" name="order" value="0"></label>
            <label><input type="checkbox" name="is_active" value="1" checked> Active</label>
            <button class="dm-btn dm-btn--green" type="submit">Créer et ajouter au département</button>
        </form>
    </div>
    <div class="dm-card">
        <h3>Matières liées au département</h3>
        @forelse($subjects as $subject)
            <div class="dm-row">
                <strong>{{ $subject->name }}</strong>
                <small>{{ $subject->description ?? 'Aucune description' }}</small>
                @if(in_array((int) $subject->id, $linkedSubjectIds, true))<span class="dm-badge">Liée au département</span>@endif
                <form method="POST" action="{{ route('department.subjects.update', $subject->id) }}" class="dm-inline">
                    @csrf
                    <input name="name" value="{{ $subject->name }}" required>
                    <input name="description" value="{{ $subject->description ?? '' }}" placeholder="Description">
                    <input name="icon" value="{{ $subject->icon ?? '' }}" placeholder="Icône">
                    <input name="color" value="{{ $subject->color ?? '#2563eb' }}">
                    <input type="number" name="order" value="{{ $subject->order ?? 0 }}">
                    <label><input type="checkbox" name="is_active" value="1" @checked((bool)($subject->is_active ?? true))> Active</label>
                    <button class="dm-btn" type="submit">Modifier</button>
                </form>
            </div>
        @empty
            <p>Aucune matière liée au département pour le moment.</p>
        @endforelse
    </div>
</section>
</div>
