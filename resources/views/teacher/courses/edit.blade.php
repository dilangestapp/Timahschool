...
@section('page_title', 'Traiter le cours')
@section('page_subtitle', 'Corrigez, enrichissez et gardez le cours dans votre espace jusqu’au moment de publication.')
...
<section class="course-writer-card">
    <div class="course-writer-head">
        <div>
            <h2>{{ $course->title }}</h2>
            <p>{{ $course->schoolClass->name ?? '-' }} — {{ $course->subject->name ?? '-' }}</p>
        </div>
        <div class="teacher-course-card__badges">
            <span class="teacher-status teacher-status--{{ $course->status }}">{{ $course->status === 'published' ? 'Publié aux élèves' : ($course->status === 'archived' ? 'Archivé' : 'Brouillon privé') }}</span>
        </div>
    </div>

    @if($course->status === \App\Models\Course::STATUS_DRAFT)
        <div class="course-writer-form" style="background:#fff8e1;border-bottom:1px solid #ffe0a3;">
            <strong style="color:#1a237e;">Cours gardé dans votre espace enseignant.</strong>
            <p class="teacher-muted" style="margin:6px 0 0;">Il reste invisible pour les élèves et l’application mobile tant que vous ne cliquez pas sur Publier.</p>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('teacher.courses.update', $course) }}" enctype="multipart/form-data" class="course-writer-form">
        ...
        <div class="course-writer-grid">
            ...
            <div><label class="course-writer-label" for="status">Visibilité</label><select id="status" name="status" class="course-writer-select"><option value="draft" @selected(old('status', $course->status) === 'draft')>Brouillon privé — garder dans mon espace</option><option value="published" @selected(old('status', $course->status) === 'published')>Publié aux élèves</option><option value="archived" @selected(old('status', $course->status) === 'archived')>Archivé</option></select></div>
            ...
            <div class="course-writer-full"><label class="course-writer-label">Contenu éditable du cours</label><small class="course-writer-help">Traitez le cours ici à votre rythme. Vous pouvez enregistrer sans publier, puis revenir plus tard.</small>@include('teacher.courses.partials.writer', ['field'=>'content_html','target'=>'#content_html','value'=>old('content_html', $course->content_html ?? '')])</div>
        </div>
        <div class="course-word-actions">
            <button type="submit" name="action_mode" value="save_private" class="course-writer-primary">Garder dans mon espace</button>
            <button type="submit" name="action_mode" value="publish_now" class="course-writer-secondary">Publier aux élèves</button>
            <button type="submit" name="action_mode" value="archive_now" class="course-writer-secondary">Archiver</button>
            <a href="{{ route('teacher.courses.index') }}" class="course-writer-secondary" style="display:inline-flex;align-items:center;text-decoration:none;">Retour</a>
        </div>
    </form>
</section>
@endsection
